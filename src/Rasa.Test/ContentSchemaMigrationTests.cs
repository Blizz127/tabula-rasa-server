...
namespace Rasa.Test
{
    ...
    public class ContentSchemaMigrationTests
    {
        ...
        public void BootcampS1SeedsTheInitiationRowsAndRollsBackToEmptyContentTables()
        {
            ...
            // WildernessMortarByNumbers: mission 430's four destroyable launchers at the positions the client map
            // gives them, each bound to its objective with a destroying hit.
            Assert.AreEqual(4L, Scalar(connection, "SELECT COUNT(*) FROM content_placement WHERE id BETWEEN 199700 AND 199703 AND entity_class_id = 7478 AND usable_kind = 2"));
            Assert.AreEqual(4L, Scalar(connection, "SELECT COUNT(*) FROM npc_mission_objective_binding WHERE mission_id = 430 AND kind = 5 AND destroying_hit_only = 1"));
