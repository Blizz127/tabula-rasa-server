namespace Rasa.Packets.ClientMethod.Server
{
    ...
    ///
    /// One id and nothing else: the client owns the tutorial's text, artwork and audio, and all
    /// the server does is decide when to raise it. The client also raises several of these
    /// itself - the duel one from wargame.py, the waypoint one from the waypoint window - so the
    /// server is one of two sources rather than the only one.
    /// </summary>
