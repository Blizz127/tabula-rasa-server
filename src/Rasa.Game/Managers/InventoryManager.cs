...
namespace Rasa.Managers
{
    ...
    public class InventoryManager
    {
        ...
        public void HomeInventory_DestroyItem(Client client, HomeInventory_DestroyItemPacket packet)
        {
            ...
            // An id that is not an item would be dereferenced downstream.
            if (tempItem == null)
                return;
        ...
        /// <summary>
        /// Re-sends the client its whole inventory picture after a map change: the
        /// manifestation and its items survive, the client's view does not.
        /// </summary>
        public void ResendToClient(Client client)
        {
            var inventory = client.Player.Inventory;

            for (var i = 0; i < inventory.EquippedInventory.Count; i++)
                if (inventory.EquippedInventory[i] != 0)
                    ItemManager.Instance.SendItemDataToClient(client, EntityManager.Instance.GetItem(inventory.EquippedInventory[i]), false);

            for (var i = 0; i < inventory.WeaponDrawer.Count; i++)
                if (inventory.WeaponDrawer[i] != 0)
                    ItemManager.Instance.SendItemDataToClient(client, EntityManager.Instance.GetItem(inventory.WeaponDrawer[i]), false);

            for (var i = 0; i < inventory.PersonalInventory.Count; i++)
                if (inventory.PersonalInventory[i] != 0)
                    ItemManager.Instance.SendItemDataToClient(client, EntityManager.Instance.GetItem(inventory.PersonalInventory[i]), false);

            for (var i = 0; i < inventory.HomeInventory.Count; i++)
                if (inventory.HomeInventory[i] != 0)
                    ItemManager.Instance.SendItemDataToClient(client, EntityManager.Instance.GetItem(inventory.HomeInventory[i]), false);
        }
